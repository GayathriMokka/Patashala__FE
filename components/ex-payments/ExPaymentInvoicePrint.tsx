...
export interface ExPaymentInvoiceData {
  payment: {
...
interface ExPaymentInvoicePrintProps {
  data: ExPaymentInvoiceData
  onClose?: () => void
  showActions?: boolean
}

function formatAddress(school: ExPaymentInvoiceData['school']) {
  if (!school) return ''
...
export default function ExPaymentInvoicePrint({ data, onClose, showActions = true }: ExPaymentInvoicePrintProps) {
    ...
    win.document.write(`
      <!DOCTYPE html><html><head>
        ...
        <style>
          ...
          .header { display: flex; align-items: flex-start; gap: 16px; border-bottom: 2px solid #059669; padding-bottom: 16px; margin-bottom: 16px; }
  ...
  const modal = (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="ex-payment-invoice-title"
      onClick={onClose}
    >
      <div
      ...
      >
        {showActions && (
          <div
            ...
            style={paperSurfaceStyle}
          >
            <h3 id="ex-payment-invoice-title" className="text-lg font-semibold text-slate-900 truncate">
              Collection Invoice
            </h3>
        ...
        <div className="invoice-paper-scroll flex-1 overflow-y-auto min-h-0 overscroll-contain" style={paperSurfaceStyle}>
          <div ref={printRef} className="invoice-paper-content p-6 sm:p-8 text-slate-900" style={paperSurfaceStyle}>
            <div className="header flex items-start gap-4 border-b-2 border-emerald-600 pb-4 mb-6">
              {showLogo && logoSrc && (
              ...
              <div className="flex-1 min-w-0 text-left">
                ...
                <p className="text-sm text-emerald-700 font-medium mt-1">
                  {settings.tagline || 'Collection Receipt'}
                </p>
            ...
            <div className="bg-slate-50 rounded-lg p-4 mb-6 text-sm">
              <p className="font-semibold text-slate-800 mb-3">Payer Details</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
                <div>
                  <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Name</p>
                  <p className="font-medium text-slate-900 mt-0.5">{payment.payer_name}</p>
                </div>
                {payment.payer_mobile && (
                  <div>
                    <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Mobile</p>
                    <p className="font-medium text-slate-900 mt-0.5">{payment.payer_mobile}</p>
                  </div>
                )}
              ...
              <tbody>
                <tr>
                  <td className="border border-slate-200 px-3 py-2">
                    {payment.source_name || 'Collection'}
                    {payment.source_description && (
                      <span className="block text-xs text-slate-500 mt-0.5">{payment.source_description}</span>
                    )}
                  </td>
            ...
            {payment.remarks && (
              <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm">
                <p className="text-xs font-medium text-amber-800 uppercase tracking-wide mb-1">Remarks</p>
                <p className="text-slate-700 whitespace-pre-wrap">{payment.remarks}</p>
              </div>
            )}
